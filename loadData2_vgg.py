...
class IAM_words(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #img_width = np.array(img_width, dtype='int64')
        #img = torch.from_numpy(img)
        #lable = torch.from_numpy(label)
        #img_width = torch.from_numpy(img_width)
    ...
    def readImage_keepRatio(self, file_name, flip):
        ...
        img_name_split=file_name.split('-')
        url = baseDir + subdir+ img_name_split[0]+'/'+img_name_split[0]+'-'+img_name_split[1]+'/'+file_name+'.png'
        ...
        outImg = np.expand_dims(outImg,0) 
...
def loadData():
    if WORD_LEVEL:
        ...
        subname = 'line'
    if RM_BACKGROUND:
        gt_tr = 'trainset-IAM.txt'
        gt_va = 'validationset-IAM.txt'
        #gt_te = 'RWTH.iam_'+subname+'_gt_final.test.thresh'
        gt_te = 'testset-IAM.txt'
    else:
        pass
